...
export default function Upload() {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const apiResponse = await res.json(); // Parse the response from your API route
            ...
            if (res.ok) {
                ...
                if (apiResponse.redirectTo) { // Check if the API response includes redirectTo
                    ...
                    setMessages(newMessages); // Update messages and then immediately redirect
                ...
                // These lines were duplicated and would cause 'body already consumed' if they ran
                // newMessages.push("Lambda processing result:");
                // newMessages.push(JSON.stringify(apiResponse.lambdaResponse, null, 2)); // Use apiResponse.lambdaResponse
            } else {
                // errorData variable was defined inside this block's scope, removed its re-declaration
                newMessages.push(`File upload failed (server error): ${apiResponse.message || res.statusText}`);
                if (apiResponse.details) {
                    newMessages.push(`Details: ${apiResponse.details}`);
                }
            }
            
            // Removed the redundant block that processed 'data' here,
            // as 'apiResponse' is the correct parsed response.
